...
const GROWTH_STEPS = [1.00, 1.08, 1.16, 1.24, 1.32, 1.40];
...
function updateBearScale() {
  ...
  if (sizeText) sizeText.textContent = bearScale.toFixed(2);
}
...
function nextLevel() {
  ...
  // Reset growth + power progression when entering fox mode
if (currentLevel === 4) {
  bearScale = 1;
  purplePower = 0;
  heroFormActive = false;
  ...
  // Reset again when switching back to bear mode
if (currentLevel === 7) {
  bearScale = 1;
  purplePower = 0;
  heroFormActive = false;
