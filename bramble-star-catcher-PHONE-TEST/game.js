...
// Keep the character inside the play area after a phone rotation or resize.
window.addEventListener("resize", () => {
  const width = bear.offsetWidth * (bearScale || 1);
  bearX = Math.max(0, Math.min(bear.offsetLeft, gameArea.clientWidth - width));
  bear.style.left = `${bearX}px`;
});
